...
| **`.woodpecker/` workflow added/removed/renamed** | `docs/woodpecker-ci.md` workflow map + coverage inventory + timeout budget, `AGENTS.md` repo layout, `release.yml` `depends_on`, and the branch-protection contexts on `main` |
| **New package script or lefthook command**        | `docs/woodpecker-ci.md` coverage inventory — every entry point is listed there as enforced or explicitly not                                                                  |
| **Pinned CI image bumped**                        | The pin in `.woodpecker/`, `.tool-versions` — Dependabot does not see `.woodpecker/`                                                                                          |
