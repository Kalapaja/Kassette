...
@Component({
  ...
  template: `
    @if (label()) {
      <span
        class="text-xs font-[421] leading-[14px] tracking-[0.48px] uppercase text-content-primary weight-primary:text-brand-primary weight-primary-disabled:text-content-primary weight-secondary-pressed:text-brand-primary"
        >{{ label() }}</span
      >
    }
    <button
      ...
      <span class="flex items-center gap-0.5">
        <span class="flex items-center justify-center w-6 h-6" aria-hidden="true">
          <ng-content select="[slot=icon]">
            <svg
              class="w-[11.5px] h-[10px] weight-primary:fill-brand-quinary weight-primary-disabled:fill-brand-quaternary weight-secondary:fill-content-primary weight-secondary-pressed:fill-fill-primary weight-secondary-disabled:fill-brand-quaternary weight-tetriary:fill-content-primary weight-tetriary-disabled:fill-brand-border-tetriary"
              viewBox="0 0 12 10"
