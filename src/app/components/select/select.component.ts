...
@Component({
  ...
  template: `
    @if (label() && weight() === 'primary') {
      <label
        class="text-xs font-[421] leading-[14px] tracking-[0.48px] uppercase text-content-primary"
        >{{ label() }}</label
      >
    }
    <button
    ...
    >
      <span
        ...
        [class.opacity-30]="!selectedLabel()"
      >
        {{ selectedLabel() || placeholder() }}
      </span>
      <span
        class="flex items-center justify-center shrink-0 w-[11px] h-[11px] transition-transform duration-150 ease-in-out"
    ...
    @if (isOpen()) {
      <div
        class="absolute top-full left-0 z-10 mt-1 min-w-full bg-fill-primary rounded-[4px] shadow-[0_0_10px_0_oklch(0_0_0/0.25)] overflow-hidden weight-primary:rounded-lg"
        role="listbox"
      >
  ...
  handleTriggerKeyDown(e: KeyboardEvent): void {
    if (e.key === 'Escape') {
      ...
      requestAnimationFrame(() => {
        const firstOption = this.elementRef.nativeElement.querySelector('.option');
