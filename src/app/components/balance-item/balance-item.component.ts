      ...
      <div class="flex items-center justify-between flex-1 min-h-px min-w-px px-[10px]">
        <div class="flex items-center gap-[7.5px]">
          <div class="relative w-9 h-9 shrink-0">
            ...
            <div
              class="chain-badge absolute -bottom-0.5 -left-0.5 w-4 h-4 rounded-full border-[1.5px] border-fill-primary bg-fill-primary overflow-hidden flex items-center justify-center"
            >
          ...
          <div class="flex flex-col gap-[5px] justify-center">
            ...
            @if (selected() && unitPrice()) {
              <span class="inline-flex items-center gap-0.5">
                <span class="text-xs font-[421] leading-[14px] text-content-tetriary">{{
                  amount()
                }}</span>
                <span class="flex items-center justify-center w-[9px] h-[9px] shrink-0">
                  ...
                  >
                    <path
                      d="M0.5 7.79297L7.79289 0.500076"
                      stroke="#D9D9D9"
        ...
        <div class="flex items-center gap-[5px]">
          @if (cryptoValue()) {
            @if (fiatParts()) {
              <span
                class="flex items-start text-xs font-[421] leading-[14px] text-content-tetriary tabular-nums"
              >
            ...
            @if (valueParts()) {
              ...
              @if (valueMatch()) {
                <span class="flex items-baseline text-content-primary tabular-nums">
                  ...
                  <span class="text-[10px] font-medium leading-4"
                    >.{{ valueMatch()![3] ?? '00' }}</span
                  >
                </span>
              } @else {
                <span class="flex items-baseline text-content-primary tabular-nums">{{
...
export class BalanceItemComponent {
  ...
  protected ariaLabel = computed(() => {
    ...
    const valueSuffix = fp ? `, ${vl} ${fiatPartsToString(fp)}` : fv ? `, ${vl} ${fv}` : '';
