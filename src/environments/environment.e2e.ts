 ...
 *
 * No credential fallbacks are shipped in the bundle — projectId and ankrApiToken
 * come from window.__APP_CONFIG__ or import.meta.env at build time. The Playwright
 * suite mocks /public/invoice, so the invoice-load path doesn't need them; if
 * future tests exercise wallet/balance flows, stub those network calls too rather
